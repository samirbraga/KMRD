...
def make_bridge_train_step_pmap(
    *,
    loss_fn,
    grad_norm: float,
    preprocess_fn,
    ...
    def train_step(
        ...
        rng, step_rng = jax.random.split(state_f.rng)
        step_rng = jax.random.fold_in(step_rng, lax.axis_index("data"))
        x, mask = batch_to_x_mask(batch)
        x, mask = preprocess_fn(x, mask)
        ...
        grads_f, grads_b = grads
        grads_f = lax.pmean(grads_f, axis_name="data")
        grads_b = lax.pmean(grads_b, axis_name="data")
...
def train_one_epoch_bridge_pmap(
    *,
    state_f: TrainState,
    state_b: TrainState,
    train_batches: Iterable[dict[str, jnp.ndarray]],
    ...
    for batch in train_batches:
        ...
        m_f = float(jax.device_get(metrics["loss_f"])[0])
        m_b = float(jax.device_get(metrics["loss_b"])[0])
        loss_sum += m_loss
        loss_f_sum += m_f
        loss_b_sum += m_b
        n_steps += 1
...
def make_bridge_train_step_for_mode(
    *,
    loss_fn,
    grad_norm: float,
    preprocess_fn,
    ...
    if distributed:
        return make_bridge_train_step_pmap(
            loss_fn=loss_fn,
            grad_norm=grad_norm,
            preprocess_fn=preprocess_fn,
        )
    return make_bridge_train_step(
        loss_fn=loss_fn,
        grad_norm=grad_norm,
        preprocess_fn=preprocess_fn,
    )
...
def train_one_epoch_bridge_for_mode(
    *,
    state_f: TrainState,
    state_b: TrainState,
    train_batches: Iterable[dict[str, jnp.ndarray]],
    ...
    if distributed:
        return train_one_epoch_bridge_pmap(
            state_f=state_f,
            state_b=state_b,
