...
def test_extrinsic_log_exp_local_consistency_with_variable_masks() -> None:
    ...
    assert jnp.allclose(y * (1.0 - m), 0.0, atol=1e-6)
    assert jnp.max(jnp.abs((v - v_rec) * m)) < 5e-3
